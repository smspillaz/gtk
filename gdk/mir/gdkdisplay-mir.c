...
_gdk_mir_display_open(const gchar *display_name)
{
    ...
    display_mir->event_queue = g_async_queue_new_full (free_gdk_event_for_queue);
    if (pipe2 (display_mir->wakeup_pipe, O_CLOEXEC | O_NONBLOCK) == -1)
...
gdk_mir_display_finalize (GObject *object)
{
  ...
  close (display_mir->wakeup_pipe[0]);
  close (display_mir->wakeup_pipe[1]);
...
void
send_fake_focus_change_event (GdkDisplay *event,
                              GdkWindow  *focus)
{
}
...
gdk_mir_display_queue_events (GdkDisplay *display)
{
  ...
  GdkEvent *event = g_async_queue_try_pop (display_mir->event_queue);
